...
bool Scene::Raycast(Point origin, Vector direction, float maxDistance, RaycastHit& hitInfo, uint layerMask, Object* objectToIgnore)
{
    hitInfo.didHit = false;
    float closestHitT = maxDistance;

    float dir_x = direction.XComponent();
    float dir_y = direction.YComponent();

    for (Object* obj : statics)
    {
        if (obj == objectToIgnore)
        {
            continue;
        }

        if ((static_cast<unsigned int>(obj->layer) & layerMask) == 0)
        {
            continue;
        }

        Rect* bbox = static_cast<Rect*>(obj->BBox());
        if (!bbox) continue;

        float inv_dir_x = 1.0f / (dir_x + 1e-8f);
        float inv_dir_y = 1.0f / (dir_y + 1e-8f);

        float t_near_x = (bbox->Left() - origin.X()) * inv_dir_x;
        float t_far_x = (bbox->Right() - origin.X()) * inv_dir_x;

        float t_near_y = (bbox->Top() - origin.Y()) * inv_dir_y;
        float t_far_y = (bbox->Bottom() - origin.Y()) * inv_dir_y;
        ...
        float t_enter = max(t_near_x, t_near_y);
        float t_exit = min(t_far_x, t_far_y);

        if (t_enter < t_exit && t_exit > 0 && t_enter >= 0 && t_enter < closestHitT)
        {
            closestHitT = t_enter;

            hitInfo.didHit = true;
            hitInfo.distance = t_enter;
            hitInfo.objectHit = obj;
            hitInfo.hitPoint = Point(origin.X() + t_enter * dir_x, origin.Y() + t_enter * dir_y);
        }
    }

    for (Object* obj : moving)
    {
        if (obj == objectToIgnore) continue;
