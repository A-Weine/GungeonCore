...
void Renderer::BeginPixels()
{
    ...
    memset(videoMemory, 0xff000000, mappedTex.RowPitch * int(Engine::game->Height()));
}
...
{
    // Pega as coordenadas de mundo diretamente do ponto.
    int px = int(point->X());
    int py = int(point->Y());

    // Opcional, mas recomendado: Uma verificação para garantir que o ponto está dentro dos limites do MUNDO.
    // Isso evita tentar desenhar em uma área da memória de vídeo que não existe.
    if (px >= 0 && px < Engine::game->Width() && py >= 0 && py < Engine::game->Height())
    {
        // Desenha o ponto em sua coordenada de mundo exata na textura de primitivas.
        // Não há mais subtração da viewport.
...
int Renderer::ClipLine(int& x1, int& y1, int& x2, int& y2)
{
    ...
    //ViewPort& activeViewport = Engine::game->GetViewport();

    /*int min_clip_x = int(activeViewport.left);
    int min_clip_y = int(activeViewport.top);
    int max_clip_x = int(activeViewport.right - 1);
    int max_clip_y = int(activeViewport.bottom - 1);*/

    int min_clip_x = 0;
    int min_clip_y = 0;
    int max_clip_x = int(Engine::game->Width() - 1);
    int max_clip_y = int(Engine::game->Height() - 1);
...
void Renderer::DrawLine(int a1, int b1, int a2, int b2, ulong color)
{
    ...
    a1 -= int(activeViewport.left);
    b1 -= int(activeViewport.top);
    a2 -= int(activeViewport.left);
    b2 -= int(activeViewport.top);*/
...
bool Renderer::Initialize(Window * window, Graphics * graphics)
{
    ...
    graphics->device->CreateBuffer(&constBufferDesc, nullptr, &constantBuffer);
    ...
    desc.Width = uint(Engine::game->Width());
    desc.Height = uint(Engine::game->Height());
    ...
    pixelPlotSprite.x = Engine::game->CenterX();
    pixelPlotSprite.y = Engine::game->CenterY();
    ...
    pixelPlotSprite.width = uint(Engine::game->Width());
    pixelPlotSprite.height = uint(Engine::game->Height());
...
void Renderer::SetTransform(const XMMATRIX& transform)
{
    D3D11_MAPPED_SUBRESOURCE mappedBuffer;
    // Mapeia o buffer para escrita, descartando o conteúdo antigo
    graphics->context->Map(constantBuffer, 0, D3D11_MAP_WRITE_DISCARD, 0, &mappedBuffer);

    // Copia a nova matriz (transposta para o formato do HLSL) para o buffer da GPU
    *(XMMATRIX*)mappedBuffer.pData = XMMatrixTranspose(transform);

    // Libera o buffer
    graphics->context->Unmap(constantBuffer, 0);
}
...
{
    // A profundidade do background é Layer::BACK (0.99f)
    if (sprite.depth > 0.9f)
    {
        // --- DEBUG PASSO D ---
        //std::string renderer_pos = "D) Renderer Recebeu BG em: X=" + std::to_string(sprite.x) + ", Y=" + std::to_string(sprite.y) + "\n\n";
        //OutputDebugStringA(renderer_pos.c_str());
        // --- FIM DEBUG ---
    }
