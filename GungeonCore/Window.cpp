...
Point Window::ScreenToWorld(Game* level)
{
    if (!level)
    {
        return Point(-1, -1);
    ...
    float camZoom = level->GetZoom();
    float camViewLeft = level->GetViewLeft();
    float camViewTop = level->GetViewTop();

    float worldMouseX = camViewLeft + (windowMouseX / camZoom);
    float worldMouseY = camViewTop + (windowMouseY / camZoom);

    return Point(worldMouseX, worldMouseY);
}
