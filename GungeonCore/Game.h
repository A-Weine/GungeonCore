...
public:
    ...
    virtual ViewPort& GetViewport() { return viewport; }

    // Funções para a câmera GPU
    virtual Scene* GetScene() const = 0; // Você já tem esta
    virtual float GetZoom() const = 0;
    virtual float GetSinkAmount() const = 0;
    virtual float GetViewLeft() const = 0;
    virtual float GetViewTop() const = 0;
};
